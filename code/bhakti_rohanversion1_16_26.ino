...
const char* host = "us-east-1-.aws.cloud2.influxdata.com";
...
void sendDataToInfluxDB(float distance, bool motion, int buzzer, unsigned long long timestamp) {
  if (WiFi.status() != WL_CONNECTED) {
    ...
    String data = "sensors, device=patentv1, location= kornfieldlab distance=" +
    String(distance) + ",motion=" +
    String(motion ? "true" : "false") +
    ",buzzer=" + String(buzzer) + " " + 
    String(timestamp);

    String url = "https://"+ String(host) + "/api/v2write?org=" + 
...
void setup() { 
  ...
  TaskUploadLogic,    
  "UploadTask",       // Name    
  15000,             // Larger stack for HTTP operations    
  NULL,               // Parameters    
  1,                  // Priority    
  NULL,               // Task handle    
  1                   // Core 1  
...
void loop() {  // Nothing here - everything is handled by tasks  delay(1000);}
