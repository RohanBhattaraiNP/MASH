...
const char* ntpServer = "time.google.com";  // Google's NTP server (more reliable)
const char* ntpServer2 = "pool.ntp.org";     // Fallback
const char* ntpServer3 = "time.nist.gov";    // Second fallback
...
unsigned long long getInfluxTimestamp() {
  ...
  if (!getLocalTime(&timeinfo)) {
    Serial.println("Failed to obtain time - attempting resync...");
    
    // Try to reconfigure time
    configTime(gmtOffset_sec, daylightOffset_sec, ntpServer);
    delay(500);
    
    // Try again
    if (!getLocalTime(&timeinfo)) {
      Serial.println("Still failed - check NTP access!");
      return 0;
    }
  }
...
void setup() {
  ...
  // Configure NTP time BEFORE creating tasks (try multiple servers)
  Serial.println("Configuring time with Google NTP...");
  configTime(gmtOffset_sec, daylightOffset_sec, ntpServer, ntpServer2, ntpServer3);

  Serial.println("Waiting for NTP time sync...");
  
  // Wait until time is actually synced (check multiple times)
  struct tm timeinfo;
  int retries = 0;
  ...
  if (retries >= 15) {
    ...
    Serial.println("- Network may block NTP (port 123)");
    Serial.println("- Firewall restrictions");
    Serial.println("Check your network allows NTP traffic");
  } else {
    Serial.println("\nTime synced successfully!");
    printLocalTime();
  }

  // Now create tasks after time is synced
